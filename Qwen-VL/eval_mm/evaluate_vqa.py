...
ds_collections = {
    ...
    'qwen_vl_eval_vqa': {
        # 'train': 'data/vqav2/vqav2_train.jsonl',
        'test': '/home/du/new_models/Qwen-VL/eval_mm/qwen_vl_eval_vqa.jsonl',
        'metric': None,
        'max_new_tokens': 10,
    },
    'okvqa_val': {
...
class VQADataset(torch.utils.data.Dataset):
    ...
    def __init__(self, train, test, prompt, few_shot, test_type):
    ...
    def __getitem__(self, idx):
        ...
        image, question, question_id, annotation = os.path.join("evaluation", data['image']), data[
            'text'], data['question_id'], data.get('answer', None)# The image path for injecting knowledge includes:evoke_news_evaluation_imgs and evoke_entity_evaluation_imgs

        if self.test_type == "image_only":
            can_list = data['top_k_image_only'][0:self.few_shot]
        if self.test_type == "text_only":
            can_list = data['top_k_text_only'][0:self.few_shot]
        if self.test_type == "UniIR":
            can_list = data['top_k_UniIR'][0:self.few_shot]
        if self.test_type == "ground_truth":
            can_list = data['ground_truth'][0:self.few_shot]
        ...
        if self.few_shot > 0:
            ...
            for i in range(self.few_shot):
                ...
                few_shot_prompt += self.prompt.format(
                    os.path.join("/home/du/pan1/new_knowledge/real_new/all_dataset_self/jkl_data/version_1_data", self.train_data[can_list[i]]['image']),
                    "The the {} image is about an object of {}. {}".format(index_self[i], data['type'], self.train_data[can_list[i]]['conversations'][1]['value']) + "\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str, default='Qwen-VL-Chat')
    parser.add_argument('--dataset', type=str, default='qwen_vl_eval_vqa')
    parser.add_argument('--test_type', type=str, default='text_only')
    ...
    dataset = VQADataset(
        ...
        train = ds_collections.get(args.dataset, {}).get('train', None),
        test=file_path,
        prompt=prompt,
    ...
    for _, (question_ids, input_ids, attention_mask,
        ...
        for question_id, answer, annotation in zip(question_ids, answers,
                                                   annotations):
            if args.dataset in ['vqav2_val', 'vqav2_testdev', 'qwen_vl_eval_vqa','okvqa_val', 'textvqa_val', 'vizwiz_val']:
                outputs.append({
                    'question_id': question_id,
                    'text': answer,
    ...
    if torch.distributed.get_rank() == 0:
        ...
        results_file = f'{checkpoint_name}_{args.dataset}_{args.test_type}_{args.few_shot}_s{args.seed}.json'
        ####################
